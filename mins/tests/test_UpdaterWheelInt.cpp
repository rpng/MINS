...
// Verifies ComputeNoiseQ3D assigns noise_w/noise_v to the measured DOFs (yaw=2, forward=3)
// and noise_p to the constrained DOFs (roll=0, pitch=1, lateral=4, vertical=5).
// Uses distinct noise values so a wrong assignment produces a wrong value — catches the
// index-swap bug where roll got noise_w and yaw got noise_p.
TEST(WheelNoiseQ3D, Ang_noise_on_correct_axes) {
    const double noise_w = 0.05, noise_v = 0.03, noise_p = 0.001, b = 1.0, dt = 0.01;
    Eigen::Matrix<double, 6, 6> Q = UpdaterWheel::ComputeNoiseQ3D("Wheel3DAng", noise_w, noise_v, noise_p, b, dt);
    EXPECT_DOUBLE_EQ(Q(0, 0), noise_p * noise_p / dt);  // roll  — constrained
    EXPECT_DOUBLE_EQ(Q(1, 1), noise_p * noise_p / dt);  // pitch — constrained
    EXPECT_DOUBLE_EQ(Q(2, 2), noise_w * noise_w / dt);  // yaw   — measured
    EXPECT_DOUBLE_EQ(Q(3, 3), noise_v * noise_v / dt);  // fwd x — measured
    EXPECT_DOUBLE_EQ(Q(4, 4), noise_p * noise_p / dt);  // lat y — constrained
    EXPECT_DOUBLE_EQ(Q(5, 5), noise_p * noise_p / dt);  // vert z — constrained
}

// Verifies Wheel3DLin propagates raw measurement noise through J * Q_raw * J^T.
// omega = (vr-vl)/b and vx = (vr+vl)/2 are linear functions of [vl, vr] with
// independent noise noise_v each, so:
//   J = [[-1/b, 1/b], [1/2, 1/2]]
//   Q_derived = J * (nv^2/dt * I) * J^T
// This is an independent derivation — not the same formula as the code.
TEST(WheelNoiseQ3D, Lin_noise_matches_independent_J_propagation) {
    const double noise_v = 0.04, noise_p = 0.001, b = 0.8, dt = 0.01;

    // Independent derivation: propagate raw [vl, vr] noise through the mixing Jacobian.
    Eigen::Matrix2d J;
    J << -1.0 / b, 1.0 / b,
          0.5,     0.5;
    Eigen::Matrix2d Q_raw = (noise_v * noise_v / dt) * Eigen::Matrix2d::Identity();
    Eigen::Matrix2d Q_derived = J * Q_raw * J.transpose();

    Eigen::Matrix<double, 6, 6> Q = UpdaterWheel::ComputeNoiseQ3D("Wheel3DLin", 0.0, noise_v, noise_p, b, dt);
    EXPECT_DOUBLE_EQ(Q(0, 0), noise_p * noise_p / dt);  // roll  — constrained
    EXPECT_DOUBLE_EQ(Q(1, 1), noise_p * noise_p / dt);  // pitch — constrained
    EXPECT_DOUBLE_EQ(Q(2, 2), Q_derived(0, 0));          // yaw   — must match J*Q*J^T
    EXPECT_DOUBLE_EQ(Q(3, 3), Q_derived(1, 1));          // fwd x — must match J*Q*J^T
    EXPECT_DOUBLE_EQ(Q(4, 4), noise_p * noise_p / dt);  // lat y — constrained
    EXPECT_DOUBLE_EQ(Q(5, 5), noise_p * noise_p / dt);  // vert z — constrained
}

TEST(WheelNoiseQ3D, Cen_noise_on_correct_axes) {
    const double noise_w = 0.05, noise_v = 0.03, noise_p = 0.001, b = 1.0, dt = 0.01;
    Eigen::Matrix<double, 6, 6> Q = UpdaterWheel::ComputeNoiseQ3D("Wheel3DCen", noise_w, noise_v, noise_p, b, dt);
    EXPECT_DOUBLE_EQ(Q(0, 0), noise_p * noise_p / dt);  // roll  — constrained
    EXPECT_DOUBLE_EQ(Q(1, 1), noise_p * noise_p / dt);  // pitch — constrained
    EXPECT_DOUBLE_EQ(Q(2, 2), noise_w * noise_w / dt);  // yaw   — measured
    EXPECT_DOUBLE_EQ(Q(3, 3), noise_v * noise_v / dt);  // fwd x — measured
