...
ROSPublisher::ROSPublisher(shared_ptr<rclcpp::Node> node, shared_ptr<SystemManager> sys, shared_ptr<Options> op) : node(node), sys(sys), op(op) {
...
void ROSPublisher::publish_imu() {
  ...
  nav_msgs::msg::Odometry odomIinG = ROSHelper::ToOdometry(odom);
...
void ROSPublisher::publish_tf() {
  ...
  auto trans = ROSHelper::Pose2TF(sys->state->imu->pose(), false);
  ...
  if (sys->state->op->cam->enabled) {
    for (const auto &calib : sys->state->cam_extrinsic) {
      geometry_msgs::msg::TransformStamped trans_calib = ROSHelper::Pose2TF(calib.second, true);
  ...
  if (sys->state->op->gps->enabled) {
    for (const auto &calib : sys->state->gps_extrinsic) {
      geometry_msgs::msg::TransformStamped trans_calib = ROSHelper::Pos2TF(calib.second, true);
      trans_calib.header.frame_id = "imu";
  ...
  if (sys->state->op->wheel->enabled) {
    geometry_msgs::msg::TransformStamped trans_calib = ROSHelper::Pose2TF(sys->state->wheel_extrinsic, true);
    trans_calib.header.frame_id = "imu";
  ...
  for (const auto &C : sys->state->clones) {
    geometry_msgs::msg::TransformStamped trans = ROSHelper::Pose2TF(C.second, false);
    trans.header.frame_id = "global";
...
void ROSPublisher::publish_state() {
  ...
  geometry_msgs::msg::PoseWithCovarianceStamped poseIinM = ROSHelper::ToPoseCov(sys->state->imu->value().block(0, 0, 7, 1));
...
void ROSPublisher::publish_cam_features() {
  ...
  sensor_msgs::msg::PointCloud2 cloud = ROSHelper::ToPointcloud(feats_msckf, "global");
  pub_cam_msckf->publish(cloud);

  // Get our good SLAM features
  vector<Vector3d> feats_slam = sys->state->get_features_SLAM();
  sensor_msgs::msg::PointCloud2 cloud_SLAM = ROSHelper::ToPointcloud(feats_slam, "global");
