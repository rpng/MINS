...
SimVisualizer::SimVisualizer(shared_ptr<rclcpp::Node> node, shared_ptr<SystemManager> sys, shared_ptr<Simulator> sim) : node(node), sys(sys), sim(sim) {
...
void SimVisualizer::publish_sim_cam_features() {
  ...
  sensor_msgs::msg::PointCloud2 cloud_SIM = ROSHelper::ToPointcloud(feats_sim, "global");
