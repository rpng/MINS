...
public:
  /// Wheel updater
  UpdaterWheelOdom(std::shared_ptr<State> state);

  /// Get wheel measurement
  void feed_measurement(OdomData data);
...
private:
  ...
  bool get_bounding_data(double t_given, std::vector<OdomData> &data_stack, OdomData &data1, OdomData &data2);
   ...
   */
  static OdomData interpolate_data(const OdomData data1, const OdomData data2, double timestamp);
   ...
   */
  void preintegration_intrinsics_3D(double dt, OdomData data);
   ...
   */
  void preintegration_3D(double dt, OdomData data1, OdomData data2);

  void perform_calc(OdomData data, Eigen::Vector3d w, Eigen::Vector3d v);
   ...
   */
  bool select_wheel_data(double time0, double time1, std::vector<OdomData> &data_vec);

  /// Our history of wheel messages (time, ang_left, ang_right)
  std::vector<OdomData> data_stack;
