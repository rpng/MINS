...
#ifndef MINS_TRAJECTORY_H
#define MINS_TRAJECTORY_H
...
public:
  ...
  double trajectory_distance() {
    double dist = 0;
    Eigen::Vector3d p = gt_poses.at(0).block(0,0,3,1);
    for (auto gt : gt_poses) {
      dist += (gt.block(0,0,3,1) - p).norm();
      p = gt.block(0,0,3,1);
    }
    return dist;
  }
  ...
  double gt_dt() {
    return abs(gt_times_all.front() - gt_times_all.back());
  }

  double est_dt() {
    return abs(est_times_all.front() - est_times_all.back());
  }
protected:
  // Trajectory data (loaded from file and timestamp intersected)
  std::vector<double> est_times, gt_times, est_times_all, gt_times_all;
